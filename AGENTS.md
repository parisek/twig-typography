...
Constructor takes `(string|array $config = '', ?callable $locale_resolver = null)`. `$config` accepts three shapes — `''` (no project overrides), `'/path/to.yml'` (filesystem YAML, project overrides), `[]`/`[...]` (PHP array, project overrides, no filesystem I/O) — and in every case the bundled `typography.yml` house policy still applies; `$config` only layers project-specific overrides on top, it no longer gates whether *any* typography runs. A missing/unreadable file falls back silently to no project overrides, not to library defaults, so `parisek/styleguide` — which passes a `typography_config` path that may not exist yet — now gets the house policy (and, if it also passes `$locale_resolver`, per-language typesetting) instead of raw `Settings(true)` defaults when that file is absent; that's a behavior change worth a heads-up to that package's maintainer, not a break, since the constructor signature and file-shape contract are unchanged. `$locale_resolver` is optional and I/O-free from this package's side: a zero-arg callable returning the active locale (e.g. `cs_CZ`); omitting it disables the per-language layer entirely, matching pre-1.3.0 behavior.
